<template>
  <div class="row">
    ...
    <div class="col-6 mt-3 pe-4">
      <SimpleResultsTableComponent :ranking="leftRanking" :resultTypes="leftResultTypes" :isAnimeSeries="isAnimeSeries" :top="top" :bottom="bottom"/>
    </div>
    <div class="col-6 mt-3 ps-4"> <!-- reverse() modifies an array in-place too, so we use slice() to create a copy first -->
      <SimpleResultsTableComponent :ranking="rightRanking ?? leftRanking.slice().reverse()" :resultTypes="rightResultTypes ?? leftResultTypes" :isAnimeSeries="isAnimeSeries" :top="top" :bottom="bottom"/>
    </div>
  </div>
</template>
