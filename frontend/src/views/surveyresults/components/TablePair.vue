<template>
  <div class="row">
    <h5 class="col col-12 subsection-title mb-1">{{ title }}</h5>
    <p class="col col-12 text-center" v-if="description">{{ description }}</p>
    <div class="col col-6 mt-3 pe-4">
      <SimpleResultsTable :ranking="leftRanking" :resultTypes="leftResultTypes" :top="top"/>
    </div>
    <div class="col col-6 mt-3 ps-4"> <!-- reverse() modifies an array in-place too, so we use slice() to create a copy first -->
